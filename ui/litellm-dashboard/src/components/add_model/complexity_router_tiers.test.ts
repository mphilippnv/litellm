...
describe("custom tier derivations", () => {
  it("defaultRuleTier prefers COMPLEX and otherwise takes the first tier the set defines", () => {
    expect(defaultRuleTier(["CASUAL", "COMPLEX"])).toBe("COMPLEX");
    expect(defaultRuleTier(["CASUAL", "AUDIT"])).toBe("CASUAL");
    expect(defaultRuleTier([])).toBe("COMPLEX");
  });
